...
type UpdateStudentUniversityInput = AddStudentUniversityInput & { id: string };

export async function updateStudentUniversityAction(
  input: UpdateStudentUniversityInput,
) {
  const created = await updateStudentUniversity(input.id, {
    universityName: input.universityName.trim(),
    fieldOfStudy: input.fieldOfStudy.trim(),
    degreeProgram: input.degreeProgram.trim(),
    grade: input.grade?.trim() || undefined,
